...
제한 되는 점이 많지만, 이를 활용하는 경우가 빈번하기 때문에
...
-- 만약 문자를 붙여서 출력하고 싶다면 concat()를 활용한다.
...
-- CHAR() workvench상에 버그로 인해 모든 문자가 BLOB로 표현되고 있습니다.
...
SELECT CHAR_LENGTH('SDFSDFASDFAEAFEFWEFAWDFS');
...
-- 공백이 아닌 특정 문자를 삭제해도 구문을 지정할 수도 있습니다.
SELECT TRIM('       무야호      ');

-- 방향은 BOTH(양쪽다), LEADING(왼쪽), TRAILING(오른쪽)
-- 중 하나를 고르면 됩니다.
SELECT TRIM(TRAILING 'ㅋ' FROM 'ㅋㅋㅋㅋㅋㅋㅋㅋㄹㅇㅋㅋㅋㅋㅋㅋㅋㅋ');
...
-- 프로시저를 활용해 employees 테이블의 10001번 직원이
...
CREATE PROCEDURE checkFiveYear()
	BEGIN
        ...
        DECLARE days INT; # 오늘날짜 - 입사일 해서 결과일수 얻어오기
...
CALL checkFiveYear();
-- DROP PROCEDURE를 이용해 삭제한 후 다른 직원 번호도 체크해보세요.
...
CREATE PROCEDURE checkFiveYear2(
	...
	BEGIN
        ...
        SELECT hire_date INTO hireDATE
			FROM employees WHERE emp_no = emp_number; # 입력받은 emp_number를 활용해 번호처리
		# hire_date INTO hireDATE는
        # 위 쿼리문의 결과로 나온 10001번 직원의 hire_date의 값을 hireDATE에 저장해줍니다.
