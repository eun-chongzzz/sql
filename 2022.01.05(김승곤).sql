...
-- 이 경우 매번 id값을 사용자가 계산해서 insert해야하게때문에
-- 굉장히 불편한데 auto_increment 속성 컬럼에 걸어주면
...
-- @@auto_increment_increment를 바꿔야합니다.
...
-- 증가분은 1이 아닌 3으로 세팅하는법
SET @@auto_increment_increment = 3; -- <-MySQL 전체적용
-- 동물 3마리를 testtbl3에 추가해보세요.
INSERT INTO testtbl3 VALUES (null, '고양이', 4);
INSERT INTO testtbl3 VALUES (null, '냐옹이', 4);
...
-- 하나의 나이대만 우선 고쳐주세요.
UPDATE testtbl2 SET user_name = '덕진이' WHERE age = 10;
SELECT * FROM testtbl2;
...
-- DELETE 특정 컬럼만 삭제하는 경우가 없고 삭제할거면 전제 row를 삭제하거나
-- 삭제를 안 해 버리거나 이기 때문에
...
-- limit르르 사용하면 상위 n개만 삭제할수도 있습니다.
-- 검색 결과물 중 상위 2개만 삭제
...
-- ON DUPLICATE KEY UPDATE 구문은 쓰면 됩니다.
INSERT INTO membertbl VALUES ('LSH', '이성혁', '제주')
...
-- INSERT INTO membertbl VALUES ('KDJ', '김덕진', '창원')ON DUPLICATE KEY UPDATE user_name = '김덕진', addr = '창원';
SELECT * FROM membertbl;
...
-- 작년 초에도 유명 게임사에서 DB르르 날려먹는 사건이 발생한 만큼
...
-- 계정 접속 방법
-- 커맨드라인(CMD) 접속시 mysql -u 계정명 -p 로 연결 가능
