...
-- ORDER BY는 결과물의 개수나 종류에는 영향을 미치치 않치만
...
-- 아래 코드는 지역 종류 5개(데이터 개수는 11개)를 파악하기 어렵습니다.
SELECT addr FROM usertbl;

-- 중복갑을 하나로 출력하기 위해 출력컬럼 앞에 distinct를 붙입니다.
...
SELECT * FROM employees limit 30, 510;

-- GROUP BY는 같은 데이터를 하나의 그룹으로 묶어줍니다.
-- 조건절 WHERE이 아닌 HAVING절로 처리하게 됩니다.
...
-- SELECT 컬럼면, 집계함수(컬럼명2)... FROM 테이블명 GROUP BY 묶을컬럼명;
...
-- 각 유저별 구매 물품의 평균가를 구해보세요(총액 아님) 평군은 avg()로 합니다.
...
-- WHERE처럼 HAVING 이라고 적어서 조건식을 붙여보세요.
SELECT user_id, sum(price * amount) as'총 구매액' FROM buytbl
 GROUP BY user_id HAVING 250000 < sum(price * amount) ;
